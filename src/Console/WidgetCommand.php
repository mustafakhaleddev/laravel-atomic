...
class WidgetCommand extends Command
{
     ...
     * @var string
     */
    protected $signature = 'atomic:widget {name}';
    ...
    public function handle()
    {
        ...
        (new Filesystem)->copyDirectory(
            __DIR__ . '../../stubs/widget-stubs',
            $this->fieldPath()
        );


        // Page.php replacements...
        $this->replace('{{ namespace }}', $this->fieldNamespace(), $this->fieldPath() . '/src/Widget.stub');
        $this->replace('{{ class }}', $this->fieldClass(), $this->fieldPath() . '/src/Widget.stub');

        (new Filesystem)->move(
            $this->fieldPath() . '/src/Widget.stub',
            $this->fieldPath() . '/src/' . $this->fieldClass() . '.php'
        );

        // FieldServiceProvider.php replacements...
        $this->replace('{{ namespace }}', $this->fieldNamespace(), $this->fieldPath() . '/src/WidgetServiceProvider.stub');
        $this->replace('{{ class }}', $this->fieldClass(), $this->fieldPath() . '/src/WidgetServiceProvider.stub');

        (new Filesystem)->move(
            $this->fieldPath() . '/src/WidgetServiceProvider.stub',
            $this->fieldPath() . '/src/WidgetServiceProvider.php'
        );

        // PageController.php replacements...
        $this->replace('{{ namespace }}', $this->fieldNamespace(), $this->fieldPath() . '/src/Http/Controllers/WidgetController.stub');

        (new Filesystem)->move(
            $this->fieldPath() . '/src/Http/Controllers/WidgetController.stub',
            $this->fieldPath() . '/src/Http/Controllers/WidgetController.php'
        );
    ...
    {
        return base_path('atomic-components/widgets/' . $this->fieldClass());
    }
    ...
    {
        return 'atomic-components/widgets/' . $this->fieldClass();
    }
